...
from keras.layers import Flatten,TimeDistributed,LSTM,Reshape
...
#onfig = tf.ConfigProto(gpu_options=gpu_options)
...
def rcnn():
   '''CNN First'''
   cnn = Sequential()
   cnn.add(Convolution2D(30, (5, 5),strides=(1,1), padding='valid', input_shape=(X_train.shape[1], X_train.shape[2], X_train.shape[3]), activation='relu'))
   ...
   cnn.add(Dense(1242*375,activation='linear'))
   cnn.compile(loss='mean_squared_error', optimizer='adam', metrics=['mse', 'mae', 'mape', 'cosine'])
   return cnn
...
print('Fitting model')
# Fit the model
history=model.fit(X_train, y_train,validation_data=(X_test, y_test), nb_epoch=10, batch_size=16, verbose=2)
...
#Test new image
test_image=image_depth.rgb_read(r"0000000118.png")
test_image=X_test[i].reshape(1,X_test[i].shape[0],X_test[i].shape[1],X_test[i].shape[2])
y_est=model.predict(test_image)
y_est=y_est.reshape((X_train.shape[1],X_train.shape[2]))*255 #De-normalize for depth viewing
image_depth.heatmap(y_est)
